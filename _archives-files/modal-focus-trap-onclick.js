...
const SelectorEngine = {
  ...
  isJQueryElement(object) {
    return this.isElement(object) && typeof object.jquery !== 'undefined' && object.jquery.indexOf('ajax') !== -1;
  },
  isElement(object) {
  ...
  isArray(element) {
    return element && typeof element === 'object' && Array.isArray(Array.from(element));
  },
  isNodeList(nodes) {
    return nodes && typeof nodes === 'object' && NodeList.prototype.isPrototypeOf(nodes) && nodes instanceof NodeList;
  },
  getElement(element) {
  ...
  getElementAll(element) {
    ...
    if (SelectorEngine.isArray(element) || SelectorEngine.isNodeList(element)) {
      return element;
    }
...
const ModalFocusTrap = new (function () {
   ...
   * @param {(String)} className (Optional) Must be a CSS classname indicating a Modal or dialog is open e.g show, active, etc.
   *
   * @example
   *
   *    <button type="button" title="Add" onclick="ModalFocusTrap.ActivateOnClick('#modal-id', this)">Add</button>
   *    <button type="button" title="Add" onclick="ModalFocusTrap.ActivateOnClick('#modal-id', this, 'show')">Add</button>
   *
   * @return {void}
   */
   ...
   * The addEventListener handles returning focus back to the element triggering Modal.
   *
  ...
  this.SetFocusTargets = function (focusTargetElements) {
    focusTargetElements = SelectorEngine.getElementAll(focusTargetElements);
